...
class ThisTemplateHelper
{
	...
	public function isHome()
	{
		...
		return (boolean)$menu->home;
	}

	public function isVacatureItem()
	{
		$app = JFactory::getApplication();
		$menu = $app->getMenu()->getActive();
		$pageclass = '';
		...
		if (is_object($menu))
		{
			...
			return (boolean)$pageclass->vacature-item;
		}
	}
	...
	public function loadCss($template)
	{
		...
		switch($cssmode)
		{
			case 'lessjs':
				...
				$doc->addScript($templateUrl.'/js/cloudflare/less.compiler.params.js');
				//$doc->addScript('//cdnjs.cloudflare.com/ajax/libs/less.js/' . $versionLess . '/less.min.js');
				$doc->addScript($templateUrl.'/js/less/less.min.js');
				break;
	...
	public function getFilterJS($template)
	{
		// Determine the variables
		$doc				= $this->getDocument();
		$templateUrl		= 'templates/'.$template->template;

		$doc->addScript($templateUrl.'/js/filterjs/json_query.js');
		$doc->addScript($templateUrl.'/js/filterjs/filter.js');
	}

	public function getFilterJSPartners($template)
	{
		// Determine the variables
		$doc				= $this->getDocument();
		$templateUrl		= 'templates/'.$template->template;

		$doc->addScript($templateUrl.'/js/filterjs/filter.partners.js');
	}
}
